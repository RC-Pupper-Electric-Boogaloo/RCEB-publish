...
import ProgressBar from 'react-native-progress/Bar'; // Add react-native-progress for better progress indicators
...
const AchievementScreen = ({ navigation }) => {
    ...
    const achievementList = [
        ...
        { id: 4, name: "Bonus Pupper!", requirement: "Get 10 skins", image: require('../assets/rcBonusPuppy.png'), progress: 0, goal: 10 },
    ...
    const updateAchievements = (stats, scoresArray, savedSkins) => {
        ...
        const updatedAchievements = achievementList.map(achievement => {
            ...
            switch (achievement.id) {
                ...
                case 5: progress = skinCount === 12 ? 1 : 0; break;
    ...
    const unlockSkin = async (skinIndex, achievementName) => {
        try {
            ...
            Alert.alert("Achievement Unlocked", `You unlocked "${achievementName}"!`);
        } catch (error) {
    ...
    const handleAchievementClick = (achievement) => {
        ...
        const rewards = {
            1: { skinIndex: 8, name: "Golden Puppy" },
            2: { skinIndex: 9, name: "Timekeeper Dachshund" },
            3: { skinIndex: 10, name: "Pirate Corgi" },
            4: { skinIndex: 11, name: "Bonus Pupper" },
            5: { skinIndex: 12, name: "Sergeant Woofer" },
            6: { skinIndex: 13, name: "Merchant Beagle" },
            7: { skinIndex: 14, name: "OG RC Pupper" },
            8: { skinIndex: 15, name: "Random Puppy" },
        };

        const reward = rewards[achievement.id];
        if (reward) unlockSkin(reward.skinIndex, reward.name);
    };
